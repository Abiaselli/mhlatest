...
class DynamicRuleEmbedding(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_frequencies=100, max_rules=1000, device=device):
        super().__init__()
        self.vocab_size = vocab_size
        self.embedding_dim = embedding_dim
        ...
        # 🔹 Full-size Rule Matrix: No more dynamic growth
        self.rule_transform = nn.Parameter(torch.randn(max_rules, embedding_dim, embedding_dim, device=device))
        self.rule_scores = torch.zeros(max_rules, device=device)

        # 🔹 Token-to-rule assignments
        self.register_buffer("token_rules", torch.randint(0, max_rules, (vocab_size,), device=device))
    ...
    def forward(self, token_ids):
        ...
        # Fourier embeddings
        x = token_ids.float().unsqueeze(-1) / self.vocab_size
        ...
        # Rule application
        rule_indices = self.token_rules[token_ids]
        rule_matrices = self.rule_transform[rule_indices]
        transformed_embeds = torch.einsum("bsd,bsde->bse", base_embeds, rule_matrices)

        return transformed_embeds

    def update_rule_scores(self, token_ids, loss_diff):
        rule_indices = self.token_rules[token_ids].detach().requires_grad
...
class TimeAwareMultiHeadLatentAttention(nn.Module):
    ...
    def forward(self, x, memory=None):
        ...
        # Multi-head shape (batch, heads, seq, head_dim)
        q = q.view(batch_size, seq_len, self.num_heads, -1).transpose(1, 2)
        k_reconstructed = k_reconstructed.view(batch_size, seq_len, self.num_heads, -1).transpose(1, 2)
        v_reconstructed = v_reconstructed.view(batch_size, seq_len, self.num_heads, -1).transpose(1, 2)
        ...
        # 🔥 Time decay (auto-generated timestamps)
        if self.use_wallclock_time:
            token_timestamps = self.get_timestamps(x)  # shape: (batch, seq_len)
            time_diffs = torch.abs(token_timestamps.unsqueeze(2) - token_timestamps.unsqueeze(1))
            time_decay = torch.exp(-self.lambda_decay * time_diffs)
            time_decay = time_decay.unsqueeze(1).expand(-1, self.num_heads, -1, -1)
...
class Rule_Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, lambda_decay=0.01, dropout=0.1, compression_factor=4, num_frequencies=100, max_rules=set_number_rules):
        ...
        self.encoder_layers = nn.ModuleList([
            TimeAwareMultiHeadLatentAttention(embedding_dim, num_heads, self.d_latent, lambda_decay)
                    ...
                    with torch.amp.autocast(device, dtype=torch.float16):  # Enable mixed precision
                        ...
                        if self.use_genetic_algo == "Genetic Algorithm":
                            ...
                            if (batch_idx + 1) % accumulation_steps == 0:     
                                    ...
                                    for opt in optimizers:
                                        scaler.unscale_(opt)
                                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1000.0)  # ✅ Clip gradients for stability
                                    ...
                                    if architecture == "Rule Transformer":
                                        ...
                                        with torch.no_grad():
                                            baseline_output = self.model(batch_input_ids, decoder_input)
                                            baseline_loss = loss_fn(baseline_output.view(-1, self.vocab_size), target_labels.view(-1))

                                        # Do rule update
                                        self.model.embedding.replace_rule()

                                        # Evaluate new rule
                                        with torch.no_grad():
                                            new_output = self.model(batch_input_ids, decoder_input)
                                            new_loss = loss_fn(new_output.view(-1, self.vocab_size), target_labels.view(-1))

                                        # Compare
                                        loss_diff = baseline_loss.item() - new_loss.item()
                                        self.model.embedding.update_rule_scores(batch_input_ids, loss_diff)
