...
tokenizer_path = r"/home/austin/vccode/slidertransformer-main/models/cascade1"  
...
class MatMulFreeLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, prev_node_output=None, src_mask=None, is_final_node=False):
        ...
        # Cross-node attention (global attention) - apply only if there is a previous node
        if prev_node_output is not None:
            output, attention_weights = self.cross_node_attention(output, prev_node_output, prev_node_output, attn_mask=None)
...
def main():
    ...
    parser.add_argument('--data', type=str, default=r"/home/austin/Sky-T1_data_17k", help='Path to JSON data')
    ...
    #model = CascadeTransformerMM( num_nodes, vocab_size, embed_size, embed_size, num_heads, eps=1e-8)
