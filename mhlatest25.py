...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output):
        attn_out, _ = self.self_attn(x)  # Decoder self-attention
        x = self.norm1(x + self.dropout(attn_out))
...
class Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, lambda_decay=0.01, dropout=0.1, compression_factor=4, num_frequencies=100, max_rules=100):
        super().__init__()
        self.embed_size = embedding_dim
...
def generate_2(model, prompt, seq_len, device, chunksize=30, max_generated=120):
                    model.eval()
                    with torch.no_grad():
                        # Tokenize the prompt and move to the correct device.
                        ...
                        tgt_ids = torch.tensor([[bos_token]], device=device)
                        tgt_ids = torch.cat([tgt_ids, input_ids], dim=1)
                        tgt_ids_length = tgt_ids.shape[1]
                        tgt_pad_length = (chunksize - (tgt_ids_length % chunksize)) % chunksize  # Only pad if needed
                        if tgt_pad_length>0:
                            tgt_tips = torch.full((1, pad_length-1), pad_token_id, dtype=torch.long).to(device)
                        tgt_ids = torch.cat([tgt_ids, tgt_tips], dim=1)
                        logging.info(f"tgt_ids: {tgt_ids}")
                        ...
                        for _ in range(seq_len - input_ids.size(1)):
                            ...
                            # Apply repetition penalty while excluding special tokens like PAD (0)
                            for token in set(generated_text[0].tolist()):
                                if token not in [pad_token_id, tokenizer.bos_token_id, tokenizer.eos_token_id]:
                                    next_token_logits[0, token] /= repetition_penalty
                            ...
                            logging.debug(f"next_token_id: {next_token_id}")
                            # Append the new token to the target sequence.
                            tgt_ids = tgt_ids[tgt_ids != pad_token_id].unsqueeze(0)
                            input_ids = torch.cat([tgt_ids, next_token_id], dim=-1)
                            tgt_ids = torch.cat([tgt_ids, next_token_id], dim=1)
                            logging.debug(f"tgt_ids: {tgt_ids}")
                            ...
                            if pad_length > 0:
                                ...
                                input_ids = torch.cat([input_ids, pad_tokens], dim=1)
                            if pad_tgt > 0:
                                pad_tgt = torch.full((1, tgt_pad_length), pad_token_id, dtype=torch.long).to(device)
                                tgt_ids = torch.cat([tgt_ids, pad_tgt], dim=1)
                            logging.debug(f"input_ids padded: {input_ids}")                                
                            generated.append(base_tokenizer.decode(next_token_id[0].tolist()))
                            logging.debug(f"generated_text: {generated_text}")
                            print(tgt_ids)
                            # 🔹 Convert tensor to integer
...
def main():
    ...
    parser.add_argument('--max_seq_length', type=int, default=120, help='Fixed maximum sequence length')
    ...
    generated_text = generate_2(model,prompt, seq_length, device)
