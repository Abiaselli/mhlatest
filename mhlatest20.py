...
def generate(model, input_text, max_seq_length, device, chunk_size=64, max_generated=1024):
                    ...
                    with torch.no_grad():
                        ...
                        for _ in range(max_generated):
                            ...
                            print(output.shape)
                            # 🔹 Select next token (greedy decoding for now)
                            next_token = torch.argmax(output[:, -1, :], dim=-1, keepdim=True)
                            next_token_count +=1
                            ouput.append(next_token)
                            # 🔹 Stop generation if EOS token appears
                            if next_token.item() == eos_token or next_token_count > max_generated:
                                break
                            # 🔹 Append new token and maintain chunk alignment
                            input_ids = torch.cat([input_ids[:, -(chunk_size-1):], next_token], dim=1)
                            print(f"🔹 New token: {next_token.item()}, Updated input size: {input_ids.shape}")
                            print(output)

                        # 🔹 Decode final output
                        generated_text = tokenizer.decode(ouput)
