...
class Transformer_Model(nn.Module):
    ...
    def decode_tgt(self, tgt_ids, memory):
        ...
        output = self.encoder_layers(
            ...
            memory_key_padding_mask=None  # we pre-masked above
        )
...
def train_tokenwise_stepwise(model, dataset, tokenizer, optimizer, loss_fn, batch_size, device, max_src_len=1024):
    model.train()
    total_loss = 0
...
def generate_autoregressive(model, prompt, tokenizer, max_tokens=50, device="cuda"):
    ...
    with torch.no_grad():
        ...
        for step in range(max_tokens):
            ...
            if next_token.item() == eos_id or step > max_tokens:
                break

        output_tokens = decoder_tokens.squeeze(0).tolist()
