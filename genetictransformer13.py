...
class Transformer_Model(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads, seq_length, device):
        super().__init__()
                            ...
                            while loss_diff >= 0:
                                ...
                                try:
                                    if architecture == "Reasoning Model LNS":
                                        ...
                                        output = self.model(val_input, val_dec_input)

                                    else:
                                        output = checkpoint.checkpoint(self.model, val_input, val_dec_input,  use_reentrant=False)
                                except Exception as e:
                                        raise ValueError(f"forward pass failed for {str(e)}")
