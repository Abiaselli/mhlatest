...
class RuleTransform(nn.Module):
    def __init__(self,vocab_size, embedding_dim, max_rules, device):
        ...
        self.token_rules = nn.Parameter(torch.randn(embedding_dim, max_rules), requires_grad=True)
    ...
    def forward(self, hidden_states):
        ...
        #rule_matrices = (self.token_rules*self.rule_transform).to(device)  # (batch, seq, d, d)
        rule_matrices = torch.tensordot(self.token_rules, self.rule_transform, dims=1)
    ...
    def optimized_rule_replacement(self, model, val_input, val_target, decoder_input, loss_fn, k=5):
        ...
        base_idx = top_k[torch.randint(0, k, (1,))].item()
        base_rule = self.rule_transform[base_idx].detach().clone().requires_grad_(True)

        learning_rates = [1e-2, 1e-3, 1e-4]
        candidates = []

        for lr in learning_rates:
            candidate = base_rule.clone().detach().requires_grad_(True)
        ...
        if best_idx is not None:
            with torch.no_grad():
                self.rule_transform[best_idx] = best_candidate
                self.rule_scores[best_idx] = 0.0
            print(f"✅ Replaced rule at index {best_idx} using optimizer-based update with val_loss={best_loss:.4f}")
...
def train_model(model, dataloader, optimizer, criterion, device):
    ...
    for batch_idx, (src, target) in enumerate(dataloader):
        ...
        model.rule_transform.optimized_rule_replacement(model, val_input, val_target, val_dec_input, criterion)
