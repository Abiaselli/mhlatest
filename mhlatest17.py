...
def collate_fn(batch, max_length, tokenizer):
    ...
    # ✅ Proper tensor handling: use .clone().detach() before wrapping in torch.tensor
    src_batch = [torch.tensor(seq[:max_length].clone().detach(), dtype=torch.long).clamp(0, tokenizer.vocab_size - 1) for seq in src_batch]
    tgt_batch = [torch.tensor(seq[:max_length].clone().detach(), dtype=torch.long).clamp(0, tokenizer.vocab_size - 1) for seq in tgt_batch]
