    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for t in range(2, max_len):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                ...
                pred_logits = logits[:, -1, :][active_mask]
                expected_token = tgt_ids[:, -1][active_mask]
                step_loss = loss_fn(pred_logits, expected_token)
                step_loss.backward()
                #print(f"[Step {t}] src: {src.shape}, tgt: {tgt_ids.shape}, pred logit: {pred_logits.shape}, pred target: {expected_token.shape}")
...
def train_sliding_window_batchwise(model, dataset, tokenizer, optimizer, loss_fn, batch_size, seq_len, device):
    model.train()
    total_loss = 0
    pad_token_id = tokenizer.pad_token_id or 0
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        # Tokenize and pad full sequences: [BOS] query [SEP] target [EOS]
        full_batch_tokens = build_training_tokens_batch(batch, tokenizer).to(device)
        batch_size, max_len = full_batch_tokens.size()
        ...
        for t in range(2, max_len):
            start = max(0, t - seq_len)
            src = full_batch_tokens[:, start:t]                 # [B, seq_len] or shorter initially
            tgt_ids = full_batch_tokens[:, start + 1:t + 1]     # shifted by 1

            if tgt_ids.size(1) == 0:
                continue  # skip if target is empty

            # Align shapes for last prediction
            if src.size(1) != tgt_ids.size(1):
                min_len = min(src.size(1), tgt_ids.size(1))
                src = src[:, -min_len:]
                tgt_ids = tgt_ids[:, -min_len:]

            active_mask = (tgt_ids[:, -1] != pad_token_id)  # last token is the prediction target
            if active_mask.sum().item() == 0:
                continue


            with torch.autocast(device_type='cuda', dtype=torch.float16):
                # Checkpointed forward: custom wrapper
                def forward_fn(src, tgt_ids):
                    return model(src, tgt_ids, mode="train")

                logits = cp.checkpoint(forward_fn, src, tgt_ids, use_reentrant=False)
                pred_logits = logits[:, -1, :]  # only the last token step


                # Select active examples (non-pad targets)
                pred_logits = pred_logits[active_mask]
                expected_token = tgt_ids[:, -1][active_mask]
                #expected_token = tgt_ids[active_mask]
                print(f"[Step {t}] src: {src.shape}, tgt: {tgt_ids.shape}, pred target: {expected_token.shape}")
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #avg_loss = train_sliding_window_batchwise(model, dataset, tokenizer, optimizer, criterion, args.batch_size, seq_len, device)
